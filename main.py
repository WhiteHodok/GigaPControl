...
def start(message):
    ...
    image_path = os.path.join(os.getcwd(), 'aboba.jpg')
    with open(image_path, 'rb') as photo:
        bot.send_photo(chat_id, photo)

    bot.send_chat_action(chat_id, 'typing')
    time.sleep(1)
    bot.send_message(chat_id, "Привет! Я бот управления компьютером. Чем могу помочь?")

    bot.send_chat_action(chat_id, 'typing')
    time.sleep(1)
    bot.send_message(chat_id, "Можете посетить мой профиль на GITHUB. Тут всегда много ништяковых ботов.:")
    bot.send_message(chat_id, "https://github.com/whitehodok")

    bot.send_chat_action(chat_id, 'typing')
    time.sleep(1)
    bot.send_message(chat_id, "Также вы можете связаться со мной в Telegram:")
    bot.send_message(chat_id, "https://t.me/whitehodok")
